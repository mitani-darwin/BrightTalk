...
backup_database() {
    ...
    for db_file in "${databases[@]}"; do
        ...
        ssh -p 47583 -i $SSH_KEY_PATH ec2-user@52.192.149.181 "docker exec $container_name sqlite3 $source_db 'PRAGMA wal_checkpoint(FULL);'" 2>/dev/null
        
        # SQLite3 .backupを実行
        if ssh -p 47583 -i $SSH_KEY_PATH ec2-user@52.192.149.181 "docker exec $container_name sqlite3 $source_db '.backup /tmp/$backup_filename' && echo 'Backup created successfully'" 2>/dev/null | grep -q "Backup created successfully"; then
            
            echo_info "バックアップ作成成功、圧縮とファイル転送を開始..."
            
            # gzipで圧縮してローカルに転送
            if ssh -p 47583 -i $SSH_KEY_PATH ec2-user@52.192.149.181 "docker exec $container_name test -f /tmp/$backup_filename && docker exec $container_name gzip -c /tmp/$backup_filename" > "$compressed_filename" 2>/dev/null; then
                ...
                if [ -s "$compressed_filename" ]; then
                    ...
                    ssh -p 47583 -i $SSH_KEY_PATH ec2-user@52.192.149.181 "docker exec $container_name rm -f /tmp/$backup_filename" 2>/dev/null
